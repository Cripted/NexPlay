...
if ($pdo && $id > 0) {
    try {
        ...
        $error = "Error al consultar la entrada: " . $e->getMessage();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titulo) || empty($categoria) || empty($extracto) || empty($contenido) || empty($fecha_publicacion)) {
        ...
        $imagen = $post['imagen']; // Conservar la imagen existente por defecto
        ...
        if (isset($_FILES['imagen_archivo']) && $_FILES['imagen_archivo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $uploadDir . $filename)) {
                    // Borrar la imagen anterior si no es de assets
                    if (!empty($post['imagen']) && strpos($post['imagen'], 'assets/img/') !== 0) {
                        $oldImg = __DIR__ . '/../' . $post['imagen'];
        ...
        if (empty($error)) {
            ...
            if ($pdo) {
                try {
                    // Verificar si ya existe el slug en otro artículo
                    ...
                    $stmt = $pdo->prepare("UPDATE blog_posts SET titulo = ?, slug = ?, categoria = ?, extracto = ?, contenido = ?, imagen = ?, fecha_publicacion = ? WHERE id = ?");
                    $stmt->execute([$titulo, $slug, $categoria, $extracto, $contenido, $imagen, $fecha_publicacion, $id]);
...
<title>Editar Artículo · NexPlay Admin</title>
  ...
  <div class="admin-grid">
    ...
    <section style="padding:0;">
      ...
      <div class="section-head" style="margin-bottom:24px;">
        <p class="eyebrow">Modificar Entrada</p>
        <h2>Editar Artículo</h2>
        <p>Modifica el artículo seleccionado y guarda los cambios en el servidor.</p>
      </div>
      ...
      <div class="form-panel">
        <form method="POST" action="editar_articulo.php?id=<?= $post['id'] ?>" enctype="multipart/form-data">
          
          <div class="form-row">
            <label for="titulo">Título del artículo *</label>
            <input type="text" id="titulo" name="titulo" required placeholder="Ej. Los mejores lanzamientos de consolas en 2026" value="<?= htmlspecialchars($post['titulo']) ?>">
          </div>
          ...
          <div class="two-col" style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
            <div class="form-row">
              ...
              <select id="categoria" name="categoria" required>
                ...
                <option value="Artículo" <?= ($post['categoria'] === 'Artículo') ? 'selected' : '' ?>>Artículo</option>
                <option value="Reseña" <?= ($post['categoria'] === 'Reseña') ? 'selected' : '' ?>>Reseña</option>
                <option value="Guía" <?= ($post['categoria'] === 'Guía') ? 'selected' : '' ?>>Guía</option>
                <option value="Avance" <?= ($post['categoria'] === 'Avance') ? 'selected' : '' ?>>Avance</option>
                ...
                <option value="Esports" <?= ($post['categoria'] === 'Esports') ? 'selected' : '' ?>>Esports</option>
              </select>
            </div>
            
            <div class="form-row">
              <label for="fecha_publicacion">Fecha de Publicación *</label>
              <input type="date" id="fecha_publicacion" name="fecha_publicacion" required value="<?= htmlspecialchars($post['fecha_publicacion']) ?>">
            </div>
          </div>

          <div class="form-row">
            <label for="extracto">Extracto (Resumen corto para portada) *</label>
            <input type="text" id="extracto" name="extracto" required placeholder="Resumen corto de 1 o 2 renglones" value="<?= htmlspecialchars($post['extracto']) ?>">
          </div>

          <div class="form-row">
            <label for="contenido">Contenido del Artículo *</label>
            <textarea id="contenido" name="contenido" rows="12" required placeholder="Escribe el cuerpo completo del artículo aquí..." style="font-family:inherit; resize:vertical;"><?= htmlspecialchars($post['contenido']) ?></textarea>
          </div>
          ...
          <div style="border-top:1px solid var(--line); margin: 24px 0; padding-top: 24px;">
            ...
            <p style="font-size:0.85rem; color:var(--muted); margin-bottom:16px;">Imagen actual: <code><?= htmlspecialchars($post['imagen']) ?></code></p>
            ...
            <div class="two-col" style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
              ...
              <div class="form-row">
                ...
                <input type="text" id="imagen_url" name="imagen_url" placeholder="https://ejemplo.com/imagen.jpg" value="<?= (strpos($post['imagen'], 'http') === 0) ? htmlspecialchars($post['imagen']) : '' ?>">
              </div>
            </div>
          </div>
